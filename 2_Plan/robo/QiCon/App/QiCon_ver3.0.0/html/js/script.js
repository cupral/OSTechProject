...
function ChangeTab3(tabname) {
  // 全部消す
  document.getElementById('tab3_tab1').style.display = 'none';
  document.getElementById('tab3_tab2').style.display = 'none';
  document.getElementById('tab3_tab3').style.display = 'none';
  // 指定箇所のみ表示
  if(tabname) {
    document.getElementById(tabname).style.display = 'block';
  }
...
// カーブボタン対応
 function touchCurvesButton(curvesNum){
   ...
   $intervalID = setInterval(
     function(){
       session.service("ALBehaviorManager").done(function(BeManager){
        BeManager.runBehavior("qicon_ver3/" + behaviorPath);
       });
     },1000
   );
